...
function secondShow(this_id) {
    if (this_id == "01") {
       ...
       "<p>只要你是在校大学生，并且有创业的梦想与激情，快来我们一起创业！</p>");
        ...
        $("#secondFont").css("background", "white");
        $("#secondFont").css("color", "black");
        $("#secondTitel").html("");
...
;$(document).delegate('#funLink2', 'click', function(ev){
    $('.nav-right li div').removeClass('active');
});
$(document).delegate('#unionLink2', 'click', function(ev){
    $('.nav-right li div').removeClass('active');
});
...
$('.nav-right li div').click(function(event) {
    $('.nav-right li div').removeClass('active');
...
//登录
$(document).delegate('#formBtn', 'click', function(ev){
        login();
});
...
$(document).delegate('.input_1','click', function(ev){
    $(".no_1").css("visibility","hidden");
});
$(document).delegate('.input_2','click', function(ev){
    $(".no_2").css("visibility","hidden");
});

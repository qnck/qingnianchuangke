...
if($_POST['fileContent']){
    $re = $uploader->uploadProxyStream();
}elseif ($_POST['fileByte']) {
...
class Upload{
    ...
    public function uploadProxyStream(){
        if(!$_POST['fileContent']){
    ...
    public function uploadProxyBinary(){
        $this->_fileContent = $_POST['fileByte'];
        $this->saveBinary();
    }

    private function setFile(){
        if(empty($_FILES['fileContent'])){
            throw new Exception("no file found", 1);
        }
    ...
    private function saveBinary(){
        $f = fopen('test.png', 'a');
        var_dump(fwrite($f, $this->_fileContent));
        var_dump(fclose($f));
        var_dump($this->_fileContent);
    }
